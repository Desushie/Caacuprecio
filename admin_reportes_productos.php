...
function product_report_reason_label(string $reason): string
{
    return match ($reason) {
        'precio_incorrecto' => 'Precio incorrecto',
        'desactualizado' => 'Falta actualizar',
        'sin_stock' => 'Sin stock',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'mark_reviewed' && $reportId > 0) {
        ...
        ");
        $stmt->execute([':id' => $reportId]);
    } elseif ($action === 'mark_all_reviewed' && $productId > 0) {
        $stmt = $pdo->prepare("
            UPDATE producto_reportes
            SET rep_estado = 'revisado'
    ...
    } elseif ($action === 'mark_all_discarded' && $productId > 0) {
        $stmt = $pdo->prepare("
            UPDATE producto_reportes
            SET rep_estado = 'descartado'
            WHERE productos_idproductos = :product_id
        ");
        $stmt->execute([':product_id' => $productId]);
    } elseif ($action === 'hide_product' && $productId > 0) {
        $stmt = $pdo->prepare("
            UPDATE productos
            SET pro_activo = 0
    ...
    } elseif ($action === 'restore_product' && $productId > 0) {
        $stmt = $pdo->prepare("
            UPDATE productos
            SET pro_activo = 1
            WHERE idproductos = :product_id
    ...
    header('Location: admin_reportes_productos.php?' . http_build_query($redirectParams));
    exit;
}
...
$stats = $pdo->query("
    SELECT
        ...
        COUNT(DISTINCT productos_idproductos) AS total_productos_reportados,
...
if ($q !== '') {
    $where[] = "(
        t.tie_nombre LIKE :q
        OR p.pro_nombre LIKE :q
        OR p.pro_descripcion LIKE :q
        OR p.pro_marca LIKE :q
        OR pr.rep_nombre LIKE :q
        OR pr.rep_email LIKE :q
        OR pr.rep_motivo LIKE :q
        OR pr.rep_detalle LIKE :q
        OR pr.rep_ip LIKE :q
        OR pr.rep_session_id LIKE :q
    )";
    $params[':q'] = '%' . $q . '%';
}

$whereSql = $where ? 'WHERE ' . implode(' AND ', $where) : '';
$having = "HAVING COUNT(pr.idreporte) >= :min_reportes";

$orderSql = match ($order) {
    'mas_recientes' => 'ultima_fecha_reporte DESC, cantidad_reportes DESC, p.idproductos DESC',
    'menos_reportadas' => 'cantidad_reportes ASC, ultima_fecha_reporte DESC, p.idproductos DESC',
    'tienda_asc' => 't.tie_nombre ASC, cantidad_reportes DESC, ultima_fecha_reporte DESC',
    'estado' => "CASE
        WHEN SUM(CASE WHEN pr.rep_estado = 'pendiente' THEN 1 ELSE 0 END) > 0 THEN 0
        WHEN SUM(CASE WHEN pr.rep_estado = 'revisado' THEN 1 ELSE 0 END) > 0 THEN 1
        ELSE 2
    END ASC, cantidad_reportes DESC, ultima_fecha_reporte DESC",
    default => 'cantidad_reportes DESC, ultima_fecha_reporte DESC, p.idproductos DESC',
};
...
$sql = "
    SELECT
        ...
        t.idtiendas,
        t.tie_nombre,
        COUNT(pr.idreporte) AS cantidad_reportes,
        MAX(pr.rep_fecha) AS ultima_fecha_reporte,
        SUM(CASE WHEN pr.rep_estado = 'pendiente' THEN 1 ELSE 0 END) AS pendientes_count,
        SUM(CASE WHEN pr.rep_estado = 'revisado' THEN 1 ELSE 0 END) AS revisados_count,
        SUM(CASE WHEN pr.rep_estado = 'descartado' THEN 1 ELSE 0 END) AS descartados_count,
        GROUP_CONCAT(pr.idreporte ORDER BY pr.rep_fecha DESC SEPARATOR ',') AS report_ids
    FROM producto_reportes pr
    INNER JOIN productos p ON p.idproductos = pr.productos_idproductos
...
if ($allReportIds) {
    ...
    $detailStmt = $pdo->prepare("
        ...
        INNER JOIN productos p ON p.idproductos = pr.productos_idproductos
        INNER JOIN tiendas t ON t.idtiendas = p.tiendas_idtiendas
        WHERE pr.idreporte IN ($placeholders)
        ORDER BY pr.rep_fecha DESC, pr.idreporte DESC
    ");
    $detailStmt->execute($allReportIds);

    foreach ($detailStmt->fetchAll() as $report) {
        $productKey = (int) $report['productos_idproductos'];
        $reportsByProduct[$productKey][] = $report;
    }
}

render_head('Gestión de productos reportados');
?>
  ...
  <div class="container">
    <div class="admin-hero p-4 p-lg-5 mb-4">
      <div class="row g-4 align-items-center">
        <div class="col-lg-8 position-relative z-1">
          ...
          <h1 class="display-6 fw-bold mb-3">Gestión de productos reportados</h1>
          <p class="text-body-secondary mb-0">
            Revisá productos reportados por usuarios, verificá la información publicada y decidí si deben mantenerse visibles dentro del sitio.
          </p>
        </div>
    ...
    <div class="row g-4 mb-4">
      <div class="col-sm-6 col-xl-3">
        <div class="admin-panel admin-stat p-4 h-100">
          <div class="admin-stat-label">Productos reportados</div>
          <div class="admin-stat-value"><?= number_format((int) ($stats['total_productos_reportados'] ?? 0), 0, ',', '.') ?></div>
        </div>
      </div>
      <div class="col-sm-6 col-xl-3">
        <div class="admin-panel admin-stat p-4 h-100">
    ...
    <div class="admin-panel p-4 mb-4">
      <form method="get" class="row g-3 align-items-end">
        <div class="col-lg-4">
          ...
          <input type="text" name="q" class="form-control" value="<?= h($q) ?>" placeholder="Producto, tienda, usuario o detalle...">
        </div>
    ...
    <div class="d-grid gap-4">
      <?php if ($productGroups): ?>
        <?php foreach ($productGroups as $item): ?>
          <?php
            $productId = (int) $item['idproductos'];
            $detailId = 'productReports' . $productId;
            $productStatusText = (int) $item['pro_activo'] === 1 ? 'Visible' : 'Oculto';
            $productStatusBadge = (int) $item['pro_activo'] === 1 ? 'bg-success' : 'bg-secondary';
            $reportList = $reportsByProduct[$productId] ?? [];
          ?>
          <div class="admin-panel p-4">
            <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap mb-3">
              <div>
                ...
                <h2 class="h4 fw-bold mb-1">Producto #<?= $productId ?></h2>
                <div class="small text-body-secondary">
                  Última actualización: <?= !empty($item['pro_fecha_scraping']) ? h(date('d/m/Y H:i', strtotime((string) $item['pro_fecha_scraping']))) : 'Sin fecha' ?>
                </div>
              </div>

              <div class="d-flex flex-wrap gap-2 align-items-center">
                <span class="badge <?= $productStatusBadge ?>"><?= $productStatusText ?></span>
            ...
            <div class="row g-4">
              <div class="col-lg-6">
                <div class="border rounded-4 p-3 h-100">
                  <div class="small text-body-secondary mb-2">Contenido del producto</div>
                  <div class="d-flex gap-3">
                    <div class="flex-shrink-0">
                      <img
                        src="<?= h(image_url($item['pro_imagen'], $item['pro_nombre'])) ?>"
                        alt="<?= h($item['pro_nombre']) ?>"
                        style="width: 88px; height: 88px; object-fit: cover; border-radius: 1rem;"
                      >
                    </div>
                    <div>
                      <div class="fw-semibold mb-1"><?= h($item['pro_nombre']) ?></div>
                      <?php if (!empty($item['pro_marca'])): ?>
                        <div class="small text-body-secondary mb-1">Marca: <?= h($item['pro_marca']) ?></div>
                      <?php endif; ?>
                      <div class="fw-semibold mb-2"><?= gs($item['pro_precio']) ?></div>
                      <div style="white-space: pre-line; line-height: 1.6;">
                        <?= h($item['pro_descripcion'] ?: 'Sin descripción.') ?>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="d-flex flex-wrap gap-2 mt-3">
              <a href="producto.php?id=<?= $productId ?>" class="btn btn-outline-primary rounded-pill">Ver producto</a>
              <a href="admin_productos.php?edit=<?= $productId ?>" class="btn btn-outline-primary rounded-pill">Editar producto</a>
              ...
              <?php if ((int) $item['pro_activo'] === 1): ?>
                <form method="post" class="d-inline" onsubmit="return confirm('¿Ocultar este producto?');">
                  <input type="hidden" name="product_id" value="<?= $productId ?>">
                  <input type="hidden" name="action" value="hide_product">
                  <button type="submit" class="btn btn-danger rounded-pill">Ocultar producto</button>
                </form>
              <?php else: ?>
                <form method="post" class="d-inline">
                  <input type="hidden" name="product_id" value="<?= $productId ?>">
                  <input type="hidden" name="action" value="restore_product">
                  <button type="submit" class="btn btn-outline-primary rounded-pill">Restaurar producto</button>
                </form>
              <?php endif; ?>
            ...
            <div class="collapse mt-4" id="<?= h($detailId) ?>">
              <div class="d-grid gap-3">
                <?php if ($reportList): ?>
                    ...
                    <div class="border rounded-4 p-3">
                      ...
                      <div class="row g-3">
                        <div class="col-lg-6">
                          ...
                          <div><strong>Motivo:</strong> <?= h(product_report_reason_label((string) $report['rep_motivo'])) ?></div>
                        </div>
                        <div class="col-lg-6">
                  ...
                  <?php endforeach; ?>
                <?php else: ?>
                  <div class="admin-empty">No hay más detalles disponibles para este producto.</div>
                <?php endif; ?>
              </div>
            </div>
          </div>
      ...
      <?php else: ?>
        <div class="admin-panel p-4">
          <div class="admin-empty">No se encontraron productos reportados con esos filtros.</div>
        </div>
      <?php endif; ?>
    </div>
  </div>
